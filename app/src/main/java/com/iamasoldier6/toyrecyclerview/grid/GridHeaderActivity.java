...
public class GridHeaderActivity extends BaseActivity {
    ...
    public void setupToolbar() {
        getSupportActionBar().setTitle(getString(R.string.toy_recycler_view_grid_has_header));
    ...
    private void initView() {
        ...
        final BackgroundAdapter backgroundAdapter =
                BackgroundAdapter.createBackgroundAdapter(this, IconHelper.VARIETY_ICONS, BackgroundAdapter.GREEN);
        layoutManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {

            @Override
            public int getSpanSize(int position) {
                return backgroundAdapter.isHeader(position) ? layoutManager.getSpanCount() : 1;
            }
        });
        mRvCommon.setAdapter(backgroundAdapter);
    }
